...
{
    constructor(_Socket = net.Socket, _Parser = undefined, _CacheConnection = undefined)
    {
        // super
        super();
        ...
        // Store the Custom Parser (if any) in `this`
        (_Parser) ? this._Parser = _Parser : this._Parser = undefined;
    ...
    _DataParser(data)
    {
        try {
            ...
            if(e.Status == 200)
            {
                ...
                {
                    (e.Msg != "PULSE") ? this._Parser.Func(e, this._Parser.Obj, this) : null;
                }
                else
                {
                    (e.Msg != "PULSE") ? console.log(`Client Message => [${e.Msg}]`) : null;
                }
            }
            else
            {                        
                console.log(`Non 200 Status => [${e.Status}] Client Message => [${e.Msg}]`);            
            }
        } catch (error) {
            console.error(error);
            console.log(`\n\n\nError`)
